...
class EquipmentListView(generics.ListCreateAPIView):
    ...
    * requesters get nothing — they never touch machines.
    ...
    permission_classes = [permissions.IsAuthenticated]
    # No pagination — the dispatch dialog enumerates every machine in
    # the lab. Pagination would silently drop machines off page 2 once a
    # lab has > 20 units, matching the bug that hid PVD / Ion Implanter
    # in the dispatcher's dropdown.
...
class RecipeListView(generics.ListAPIView):
    ...
    Pagination is **disabled** here on purpose: the dispatch dialog reads
    every recipe in the user's lab into a client-side ``filter()`` keyed
    by equipment_type, so a paginated response would silently drop a
    machine's recipes off page 2 (the symptom that made PVD / Ion
    Implanter dropdowns appear empty for the Thin-Film dispatcher).
    ...
    permission_classes = [permissions.IsAuthenticated]
    # No pagination — the dispatcher needs every active recipe for their
    # lab in a single response so the equipment-type filter in the
    # dispatch dialog can find every match. Lab recipe counts are in
    # the dozens, far below any sane "fetch everything" threshold.
    pagination_class = None
