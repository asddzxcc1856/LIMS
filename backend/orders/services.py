...
def split_order(order: Order, *, splits, operator, parent_sample=None) -> list:
    ...
    ``splits`` is an iterable of
    ``{sub_code, wafer_count, notes, execution_order}`` dicts. Each
    dict becomes a ``Sample`` row pinned to ``order``. When
    ...
    for entry in splits:
        ...
        try:
            execution_order = int(entry.get('execution_order', 0))
        except (TypeError, ValueError):
            raise ValidationError(f'execution_order for "{sub_code}" must be an integer.')
        if execution_order < 0:
            raise ValidationError(f'execution_order for "{sub_code}" must be ≥ 0.')
        cleaned.append({
            'sub_code': sub_code,
...
def set_sample_parameters(sample, *, operator, parameter_overrides):
